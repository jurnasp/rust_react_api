...
pub const TOKEN_SECRET: &'static str = "8Xui8SN4mI+7egV/9dlfYYLGQJeEx4+DwmSQLwDVXJg=";
...
impl<'r> FromRequest<'r> for Auth {
    ...
    async fn from_request(req: &'r Request<'_>) -> request::Outcome<Auth, Self::Error> {
        let secret = TOKEN_SECRET.to_string().into_bytes();
        if let Some(auth) = extract_auth_from_request(req, &secret
        ) {
